/*
  TMFS: Filesystem in Userspace
  Copyright (C) 2001-2007  Miklos Szeredi <miklos@szeredi.hu>

  This program can be distributed under the terms of the GNU LGPLv2.
  See the file COPYING.LIB.
*/

/* these definitions provide source compatibility to prior versions.
   Do not include this file directly! */

struct tmfs_operations_compat25 {
	int (*getattr) (const char *, struct stat *);
	int (*readlink) (const char *, char *, size_t);
	int (*getdir) (const char *, tmfs_dirh_t, tmfs_dirfil_t);
	int (*mknod) (const char *, mode_t, dev_t);
	int (*mkdir) (const char *, mode_t);
	int (*unlink) (const char *);
	int (*rmdir) (const char *);
	int (*symlink) (const char *, const char *);
	int (*rename) (const char *, const char *);
	int (*link) (const char *, const char *);
	int (*chmod) (const char *, mode_t);
	int (*chown) (const char *, uid_t, gid_t);
	int (*truncate) (const char *, off_t);
	int (*utime) (const char *, struct utimbuf *);
	int (*open) (const char *, struct tmfs_file_info *);
	int (*read) (const char *, char *, size_t, off_t,
		     struct tmfs_file_info *);
	int (*write) (const char *, const char *, size_t, off_t,
		      struct tmfs_file_info *);
	int (*statfs) (const char *, struct statvfs *);
	int (*flush) (const char *, struct tmfs_file_info *);
	int (*release) (const char *, struct tmfs_file_info *);
	int (*fsync) (const char *, int, struct tmfs_file_info *);
	int (*setxattr) (const char *, const char *, const char *, size_t, int);
	int (*getxattr) (const char *, const char *, char *, size_t);
	int (*listxattr) (const char *, char *, size_t);
	int (*removexattr) (const char *, const char *);
	int (*opendir) (const char *, struct tmfs_file_info *);
	int (*readdir) (const char *, void *, tmfs_fill_dir_t, off_t,
			struct tmfs_file_info *);
	int (*releasedir) (const char *, struct tmfs_file_info *);
	int (*fsyncdir) (const char *, int, struct tmfs_file_info *);
	void *(*init) (void);
	void (*destroy) (void *);
	int (*access) (const char *, int);
	int (*create) (const char *, mode_t, struct tmfs_file_info *);
	int (*ftruncate) (const char *, off_t, struct tmfs_file_info *);
	int (*fgetattr) (const char *, struct stat *, struct tmfs_file_info *);
};

struct tmfs *tmfs_new_compat25(int fd, struct tmfs_args *args,
			       const struct tmfs_operations_compat25 *op,
			       size_t op_size);

int tmfs_main_real_compat25(int argc, char *argv[],
			    const struct tmfs_operations_compat25 *op,
			    size_t op_size);

struct tmfs *tmfs_setup_compat25(int argc, char *argv[],
				 const struct tmfs_operations_compat25 *op,
				 size_t op_size, char **mountpoint,
				 int *multithreaded, int *fd);

void tmfs_teardown_compat22(struct tmfs *tmfs, int fd, char *mountpoint);

#if !defined(__FreeBSD__) && !defined(__NetBSD__)
#include <sys/statfs.h>

struct tmfs_operations_compat22 {
	int (*getattr) (const char *, struct stat *);
	int (*readlink) (const char *, char *, size_t);
	int (*getdir) (const char *, tmfs_dirh_t, tmfs_dirfil_t);
	int (*mknod) (const char *, mode_t, dev_t);
	int (*mkdir) (const char *, mode_t);
	int (*unlink) (const char *);
	int (*rmdir) (const char *);
	int (*symlink) (const char *, const char *);
	int (*rename) (const char *, const char *);
	int (*link) (const char *, const char *);
	int (*chmod) (const char *, mode_t);
	int (*chown) (const char *, uid_t, gid_t);
	int (*truncate) (const char *, off_t);
	int (*utime) (const char *, struct utimbuf *);
	int (*open) (const char *, struct tmfs_file_info_compat *);
	int (*read) (const char *, char *, size_t, off_t,
		     struct tmfs_file_info_compat *);
	int (*write) (const char *, const char *, size_t, off_t,
		      struct tmfs_file_info_compat *);
	int (*statfs) (const char *, struct statfs *);
	int (*flush) (const char *, struct tmfs_file_info_compat *);
	int (*release) (const char *, struct tmfs_file_info_compat *);
	int (*fsync) (const char *, int, struct tmfs_file_info_compat *);
	int (*setxattr) (const char *, const char *, const char *, size_t, int);
	int (*getxattr) (const char *, const char *, char *, size_t);
	int (*listxattr) (const char *, char *, size_t);
	int (*removexattr) (const char *, const char *);
	int (*opendir) (const char *, struct tmfs_file_info_compat *);
	int (*readdir) (const char *, void *, tmfs_fill_dir_t, off_t,
			struct tmfs_file_info_compat *);
	int (*releasedir) (const char *, struct tmfs_file_info_compat *);
	int (*fsyncdir) (const char *, int, struct tmfs_file_info_compat *);
	void *(*init) (void);
	void (*destroy) (void *);
};

struct tmfs *tmfs_new_compat22(int fd, const char *opts,
			       const struct tmfs_operations_compat22 *op,
			       size_t op_size);

struct tmfs *tmfs_setup_compat22(int argc, char *argv[],
				 const struct tmfs_operations_compat22 *op,
				 size_t op_size, char **mountpoint,
				 int *multithreaded, int *fd);

int tmfs_main_real_compat22(int argc, char *argv[],
			    const struct tmfs_operations_compat22 *op,
			    size_t op_size);

typedef int (*tmfs_dirfil_t_compat) (tmfs_dirh_t h, const char *name, int type);
struct tmfs_operations_compat2 {
	int (*getattr)	   (const char *, struct stat *);
	int (*readlink)	   (const char *, char *, size_t);
	int (*getdir)	   (const char *, tmfs_dirh_t, tmfs_dirfil_t_compat);
	int (*mknod)	   (const char *, mode_t, dev_t);
	int (*mkdir)	   (const char *, mode_t);
	int (*unlink)	   (const char *);
	int (*rmdir)	   (const char *);
	int (*symlink)	   (const char *, const char *);
	int (*rename)	   (const char *, const char *);
	int (*link)	   (const char *, const char *);
	int (*chmod)	   (const char *, mode_t);
	int (*chown)	   (const char *, uid_t, gid_t);
	int (*truncate)	   (const char *, off_t);
	int (*utime)	   (const char *, struct utimbuf *);
	int (*open)	   (const char *, int);
	int (*read)	   (const char *, char *, size_t, off_t);
	int (*write)	   (const char *, const char *, size_t, off_t);
	int (*statfs)	   (const char *, struct statfs *);
	int (*flush)	   (const char *);
	int (*release)	   (const char *, int);
	int (*fsync)	   (const char *, int);
	int (*setxattr)	   (const char *, const char *, const char *,
			    size_t, int);
	int (*getxattr)	   (const char *, const char *, char *, size_t);
	int (*listxattr)   (const char *, char *, size_t);
	int (*removexattr) (const char *, const char *);
};

int tmfs_main_compat2(int argc, char *argv[],
		      const struct tmfs_operations_compat2 *op);

struct tmfs *tmfs_new_compat2(int fd, const char *opts,
			      const struct tmfs_operations_compat2 *op);

struct tmfs *tmfs_setup_compat2(int argc, char *argv[],
				const struct tmfs_operations_compat2 *op,
				char **mountpoint, int *multithreaded, int *fd);

struct tmfs_statfs_compat1 {
	long block_size;
	long blocks;
	long blocks_free;
	long files;
	long files_free;
	long namelen;
};

struct tmfs_operations_compat1 {
	int (*getattr)	(const char *, struct stat *);
	int (*readlink) (const char *, char *, size_t);
	int (*getdir)	(const char *, tmfs_dirh_t, tmfs_dirfil_t_compat);
	int (*mknod)	(const char *, mode_t, dev_t);
	int (*mkdir)	(const char *, mode_t);
	int (*unlink)	(const char *);
	int (*rmdir)	(const char *);
	int (*symlink)	(const char *, const char *);
	int (*rename)	(const char *, const char *);
	int (*link)	(const char *, const char *);
	int (*chmod)	(const char *, mode_t);
	int (*chown)	(const char *, uid_t, gid_t);
	int (*truncate) (const char *, off_t);
	int (*utime)	(const char *, struct utimbuf *);
	int (*open)	(const char *, int);
	int (*read)	(const char *, char *, size_t, off_t);
	int (*write)	(const char *, const char *, size_t, off_t);
	int (*statfs)	(struct tmfs_statfs_compat1 *);
	int (*release)	(const char *, int);
	int (*fsync)	(const char *, int);
};

#define TMFS_DEBUG_COMPAT1	 (1 << 1)

struct tmfs *tmfs_new_compat1(int fd, int flags,
			      const struct tmfs_operations_compat1 *op);

void tmfs_main_compat1(int argc, char *argv[],
		       const struct tmfs_operations_compat1 *op);

#endif /* __FreeBSD__ || __NetBSD__ */
